...
def argument_parser():
    ...
    parser.add_argument('--seeds', type=int, default = 4, help='number of nodes')
...
if args.no_hosts:
    name += "_nohost"
MLPPATH = 'logs/{0}/{1}_MLP_checkpoint.pt'.format(name, 'ALG_GCN')
GCNPATH = 'logs/{0}/{1}_GCN_checkpoint.pt'.format(name, 'ALG_GCN')
...
def main():  
    ...
    print("Overview of adjacency martrix ", adj_matrix.shape)
    ...
    for iteration in range(1,T-1):
        ...
        with torch.no_grad():
            ...
            outputs.append(output[:num_node])
...
def evaluate(hidden_size, features, labels, idx_train, idx_val, idx_test, adj):
    ...
    print("Optimization Finished!")
    print("Test accuracy of the best model: {:.4f}".format(test_acc))
    print("Total time elapsed: {:.4f}s".format(time.time() - t_total))
