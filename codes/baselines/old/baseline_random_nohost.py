...
name = 'randomnohost'
MLPPATH = 'logs/randomnohost/b_random_MLP_checkpoint.pt'
GCNPATH = 'logs/randomnohost/b_random_GCN_checkpoint.pt'
...
def argument_parser():
    ...
    parser.add_argument('--num_hosts', type=int, default = 118,help='number of host nodes in the graph')
...
def main():  
    ...
    # initial_node_index = [[] for i in range(num_class)] # [num_class, index]
    # tmp_node_index = []
    # for i in range(num_hosts, num_hosts + num_for_label):
    #     node_class = labels_npy[i]
    #     initial_node_index[node_class].append(i) 
    # # print(initial_node_index)
    # for i in range(num_class):
    #     # print(i, len(initial_node_index[i]))
    #     if len(initial_node_index[i]) > 2:
    #         initial_node_index[i] = random.sample(initial_node_index[i],initial_num_per_class)
    #     for j in initial_node_index[i]:
    #         tmp_node_index.append(j)
    #     # print(i, len(initial_node_index[i]))
    # initial_node_index = np.array(tmp_node_index).ravel()
    ...
    #         node_entropy = get_entropy(output1[node])
    #         score[node] = node_entropy
    ...
    print("len of idx_train before node selection {}: {}".format(1,len(idx_train)))
    ...
    idx_train, idx_available = node_selection(idx_train, idx_available, batch_size= batch_size * num_class)
    print("len of idx_train after node selection {}: {}".format(1,len(idx_train)))
    
    # weight1 = 0.5*np.log(acc_val1.cpu().numpy() /(1-acc_val1.cpu().numpy() ))
    # output1 = F.softmax(model1(features_GCN[:num_node], adj),1).cpu().detach().numpy()
    # weights = [weight1]
    # outputs = [output1]
    ...
    for iteration in range(1,T-1):
        ...
        idx_train, idx_available = node_selection(idx_train, idx_available, batch_size= batch_size * num_class)
        # idx_train, idx_available = node_selection(idx_train, idx_available, score, batch_size= batch_size * num_class)
    ...
    pd.DataFrame({"num": idx_train_list, "acc": test_accs}).to_csv("logs/randomnohost/data.csv")

def node_selection(idx_train, idx_available, batch_size):
...
# def node_selection(idx_train, idx_available, score, batch_size):
#     idx_train = list(idx_train.cpu().numpy())
#     num_labeled_temp = 0
#     node_list = list(np.argsort(score))
#     print("Start to select nodes")
#     while num_labeled_temp < batch_size:
#         node_selected = node_list[-1]
#         node_list.remove(node_selected)
#         # print("To Select node {}, Score {}".format(node_selected, score[node_selected]))
...
def train(epoch, model, optimizer, features, labels, idx_train, idx_val, idx_test, record, adj = None):
    ...
    with torch.no_grad():
        ...
        acc_test = accuracy(output[idx_test], labels[idx_test])
        acc_test_umb = accuracy(output[idx_test], labels[idx_test], balanced = False)
    ...
    if epoch % 10 == 0:
        ...
        print("current test_acc_umb: {:.4f} ".format( acc_test_umb.item()))
        print("############################################################################")
...
def evaluate(hidden_size, features, labels, idx_train, idx_val, idx_test, adj):
    ...
    cm.figure_.savefig('logs/randomnohost/b_random_confusion_matrix.png')
