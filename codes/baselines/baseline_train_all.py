...
GCNPATH = 'logs/{0}/b_{1}_GCN_checkpoint.pt'.format(name, 'trainall')
...
def main():  
    ...
    # Model Evaluation 
    # pd.DataFrame({"num": [len(idx_train)], "acc": [test_acc.cpu().numpy()]}).to_csv("logs/trainall/data.csv")
    pd.DataFrame({"num": [len(idx_train)], "acc": [test_acc]}).to_csv("logs/{}}/data.csv".format(name))
